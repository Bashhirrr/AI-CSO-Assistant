...
struct VideoCallViewTest: View {
            ...
            VStack(spacing: 10) {
                ...
                if !isConnected {
                    HStack(spacing: 8) {
                        ...
                        Text("No Internet Connection").font(.caption).foregroundColor(.red)
                    }
                    .padding(.top, 10)
                }
            }
    ...
    private var activeCallView: some View {
        ZStack {
            ...
            VStack {
                Spacer()
                HStack(spacing: 40) {
                    CallControlButton(icon: isMicOn ? "mic.fill" : "mic.slash.fill", isActive: isMicOn) {
                        hapticTap()
                        isMicOn.toggle()
                        agora.muteLocalAudio(!isMicOn)
                    }
                    EndCallButton { endCall() }
                    CallControlButton(icon: isCameraOn ? "video.fill" : "video.slash.fill", isActive: isCameraOn) {
                        hapticTap()
                        toggleCamera()
                    }
                }
    ...
    private var connectionFailedView: some View {
        VStack(spacing: 20) {
            ...
            Spacer()
            Button { resetCall() } label: {
                Text("Try Again").font(.headline).padding().background(Color.gray.opacity(0.3)).cornerRadius(12)
            }
            Spacer()
        }
    }
...
struct VideoCallViewTest_Previews: PreviewProvider {
